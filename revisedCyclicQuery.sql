...
CREATE OR REPLACE PROCEDURE get_hier_data(epi_list IN VARCHAR2,result_cursor OUT SYS_REFCURSOR)  AS 
    ...
    CURSOR distinct_key_cursor IS
        SELECT DISTINCT up_id
        FROM temp_hierarchy_node_row;
        -- WHERE type = 4;        can be deleted
    ...
    FOR i IN ( 
        ...
        EXCEPTION 
            WHEN NO_DATA_FOUND THEN 
            	...
            	SELECT i.l,NULL,NULL,0 AS up_node_count 
            	from dual;
        END;
		BEGIN
            SELECT i.l AS TARGET_ppi, child AS up_id, lvl, 0 AS up_node_count ,null as up_ppi
     ...
     END LOOP;
        ...
        FOR rec IN distinct_key_cursor LOOP
            ...
            IF v_hierarchy_nodes(i).up_id = v_up_id THEN
                ...
                v_hierarchy_nodes(i).up_ppi := v_up_lvl_data.UP_ppi;
            END IF;
       		 END LOOP;
		END LOOP;
...
BEGIN 
    ...
    get_hier_data('asd,def,hgd,tre', result_cursor);
