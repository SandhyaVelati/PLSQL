...
BEGIN 
    ...
    FOR i IN ( 
        ...
        BEGIN 
            INSERT INTO temp_hierarchy_node_row (target_ppi, up_id, lvl, up_node_count)
        ...
        EXCEPTION 
            WHEN NO_DATA_FOUND THEN 
                ...
                INSERT INTO temp_hierarchy_node_row (target_ppi, up_id, lvl, up_node_count) 
            	SELECT i.l,NULL,NULL,0 AS TARGET_ppi 
            	from dual;
        END;
		BEGIN
            SELECT i.l AS TARGET_ppi, child AS up_id, lvl, 0 AS up_node_count 
        ...
        EXCEPTION 
            WHEN NO_DATA_FOUND THEN 
                -- DBMS_OUTPUT.PUT_LINE('No data found for ' || i.l); 
				SELECT i.l AS TARGET_ppi,NULL, NULL as lvl, 0 AS up_node_count   
           		 INTO v_up_lvl_data.TARGET_ppi, v_up_lvl_data.up_id, v_up_lvl_data.lvl, v_up_lvl_data.up_node_count 
                FROM DUAL;
        END; 

        -- DBMS_OUTPUT.PUT_LINE(v_up_lvl_data.TARGET_ppi || ' ' || v_up_lvl_data.up_id || ' ' || v_up_lvl_data.lvl || ' ' || v_up_lvl_data.up_node_count); 
        ...
        FOR rec IN distinct_key_cursor LOOP
            ...
            -- Output the result
        	 --    DBMS_OUTPUT.PUT_LINE('Parent: ' || v_up_clt_oid || ', Child Count: ' || v_up_node_count);
        	DBMS_OUTPUT.PUT_LINE(v_up_lvl_data.TARGET_ppi || ' ' || v_up_lvl_data.up_id || ' ' || v_up_lvl_data.lvl || ' ' || v_up_lvl_data.up_node_count); 
        END LOOP;
     END LOOP;
...
BEGIN 
    ...
    get_hier_data('asd,def,tre,hgd', result_cursor);
